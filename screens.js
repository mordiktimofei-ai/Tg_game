...
App.register('dashboard', () => `
  <div style="display:flex;flex-direction:column;height:100%;">
    <div style="padding:16px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid rgba(255,255,255,0.05);">
      ...
      <button class="btn btn-ghost" style="width:36px;height:36px;padding:0;border-radius:10px;background:var(--card);">⚙️</button>
    </div>
    <div style="padding:16px;display:flex;justify-content:space-between;align-items:center;">
      ...
      <button class="btn btn-primary" style="padding:10px 20px;background:rgba(42,171,238,0.15);color:var(--accent);border:1px solid rgba(42,171,238,0.3);font-size:14px;"
        onclick="App.tg.openInvoice('deposit',{amount:1000000000})">+ Пополнить</button>
    </div>
...
App.register('stake', () => `
  <div style="display:flex;flex-direction:column;height:100%;">
    <div style="padding:16px;display:flex;align-items:center;gap:12px;border-bottom:1px solid rgba(255,255,255,0.05);">
      <button class="btn btn-secondary" style="width:36px;height:36px;padding:0;" data-action="goDashboard">←</button>
  ...
  App.connectWS('/ws/queue', {
    ...
    queue_timeout: () => {
      ...
      App.showModal('Соперник не найден', 'Попробуйте другую ставку или пригласите друга!',
        [{text:'Изменить ставку',action:'goDashboard'}, {text:'Искать снова',action:'navigate',target:'stake'}]);
    }
  });
  ...
  <div style="display:flex;flex-direction:column;height:100%;" id="gameScreen">
    ...
    <div style="padding:16px;border-top:1px solid rgba(255,255,255,0.05);">
      ...
      <div style="display:flex;gap:10px;justify-content:center;">
        ...
        <button class="btn btn-secondary" style="padding:10px 18px;font-size:13px;" onclick="App.tg.showPopup({title:'📋 Правила',message:'• Ходите по очереди\n• 15 сек на ход\n• 3 в ряд = победа\n• Ничья = возврат ставок'})">📋 Правила</button>
      </div>
    </div>
    <div id="chat-panel" style="position:fixed;bottom:0;left:0;right:0;background:var(--card);border-radius:20px 20px 0 0;padding:16px;transform:translateY(100%);transition:transform 0.3s;z-index:50;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;"><span style="font-weight:600;">💬 Быстрые фразы</span>
        <button onclick="document.getElementById('chat-panel').classList.remove('active')" style="background:none;border:none;color:var(--text-secondary);font-size:20px;cursor:pointer;">✕</button>
      </div>
...
App.register('result', (params) => {
  ...
  <div style="min-height:100vh;position:relative;">
    ...
    <div style="padding:0 24px 32px;display:flex;flex-direction:column;gap:10px;">
      ...
      <button class="btn btn-primary btn-large" style="animation:btnSlide 0.4s ease ${isWin?'1.0':'0.9'}s both;" data-action="playAgain"><span>🔄</span><span>Играть ещё</span></button>
      <button class="btn btn-secondary btn-large" style="animation:btnSlide 0.4s ease ${isWin?'1.1':'1.0'}s both;" data-action="goDashboard"><span>🏠</span><span>В меню</span></button>
    </div>
  </div>`;
});
